...
function buildPasswordLoginUrl(autoLoginUrl) {
  const loginUrl = new URL(autoLoginUrl);
  loginUrl.pathname = loginUrl.pathname.replace(/\/index\.html$/, "/login.html");
  loginUrl.searchParams.set("protocol", loginUrl.protocol.replace(":", ""));
  loginUrl.searchParams.set("showback", "true");
  return loginUrl.toString();
}

function buildAppConfUrl(pageUrl) {
  const target = new URL(pageUrl);
  return new URL(`/api/logbox/oauth2/wap/appConf.do${target.search}`, target.origin).toString();
}
...
async function login(username, password) {
  ...
  await session.text(dynamicUrl);
  const loginUrl = buildPasswordLoginUrl(session.lastUrl || dynamicUrl);
  const config = await session.json(buildAppConfUrl(loginUrl), { method: "POST" });
  if (String(config.result) !== "0" || !config.data) {
    throw new Error(config.msg || "登录配置获取失败");
  }

  const loginPage = await session.text(loginUrl);
  const rsaKey = mustMatch(
    loginPage,
    /id=["']j_rsaKey["'][^>]*value=["']([^"']+)/,
    "RSA 公钥",
  );
  const loginConfig = config.data;

  const body = new URLSearchParams({
    apptype: "wap",
    appKey: "cloud",
    accountType: loginConfig.accountType || "02",
    dynamicCheck: "false",
    userName: `{RSA}${rsaEncode(rsaKey, username)}`,
    epd: `{RSA}${rsaEncode(rsaKey, password)}`,
    validateCode: "",
    captchaToken: "",
    returnUrl: encodeURIComponent(loginConfig.returnUrl || ""),
    isConfigurable: "true",
    isOauth2: String(loginConfig.isOauth2 ?? true),
    state: loginConfig.state || "",
    paramId: loginConfig.paramId || "",
    lt: loginConfig.lt || "",
    REQID: loginConfig.reqId || "",
  });
